#ifndef __RLS_FILTER_HPP__
#define __RLS_FILTER_HPP__
...
class RlsFilter {
    private:
        ...
        T *m_delta_R_x;
        T *m_delta_r_d;
        char *m_workspace_solveLinEq; // workspace for `Utils::Math::LinAlg::solveLinearEquation` function
    ...
    public:
        ...
        void trainHard(const T *vec_d, const T *vec_x, int Ld, T *w_opt) {
            ...
            std::fill_n(m_r_d, m_p+1, ZERO);
                ...
                puts("before loop:");
                puts("m_R_x:");
                Utils::Math::LinAlg::Debug::printComplexMat(m_p+1, m_p+1, m_R_x, format);
                puts("m_r_d:");
                Utils::Math::LinAlg::Debug::printComplexVec(m_p+1, m_r_d, format);
                puts("-----");
            #endif

            for (int n=0; n<Ld; ++n) {
                using namespace Utils::Math::LinAlg;
                ...
                addVec_inplace(m_p+1, m_r_d, m_delta_r_d);
                    ...
                    printf("n == %d:\n", n);
                    puts("m_x:");
                    Utils::Math::LinAlg::Debug::printComplexVec(m_p+1, m_x, format);
                    puts("-----");
                #endif
            }
            Utils::Math::LinAlg::solveLinearEquation(m_p+1, m_R_x, m_r_d, w_opt, m_workspace_solveLinEq);

            #if IS_DEBUGGING == true
                puts("w_opt:");
                Utils::Math::LinAlg::Debug::printComplexVec(m_p+1, w_opt, format);
            #endif
            #undef IS_DEBUGGING
        }
};
